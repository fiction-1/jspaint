((exports) => {
	...
	function MenuBar(menus) {
		...
		function MenuPopup(menu_items, { parentMenuPopup } = {}) {
			...
			const add_menu_item = (parent_element, item, item_index) => {
				...
				if (item === MENU_DIVIDER) {
					...
					let preview_img_el;
					if (item.image) {
						preview_img_el = E("img", { class: "menu-item-preview", alt: item.imageAlt || "" });
						const preview_src = typeof item.image === "function" ? item.image() : item.image;
						preview_img_el.dataset.src = preview_src;
						label_el.appendChild(preview_img_el);
					}
					...
					if (preview_img_el) {
						const preview_src = typeof item.image === "function" ? item.image() : item.image;
						if (preview_src && preview_img_el.src !== preview_src) {
							preview_img_el.src = preview_src;
						} else if (!preview_img_el.src) {
							preview_img_el.src = preview_src || "";
						}
					}
		...
		const make_menu_button = (menus_key, menu_items) => {
			...
			const is_direct_action_menu = menu_items.length === 1 && typeof menu_items[0].action === "function" && !menu_items[0].submenu;
			menu_button_el.addEventListener("pointerdown", e => {
				if (menu_button_el.classList.contains("active")) {
					if (is_direct_action_menu) {
						menu_items[0].action?.(e);
						refocus_outside_menus();
						e.preventDefault();
					} else {
						menu_button_el.dispatchEvent(new CustomEvent("release", {}));
			...
			if (menu_items.hoverMenu) {
				menu_button_el.addEventListener("pointerenter", e => {
					if (e.pointerType === "touch") {
						return;
					}
					if (top_level_menu_index === -1 && !menu_button_el.classList.contains("active")) {
						open_top_level_menu("pointerenter");
					}
				});
			}
